 ...
 */
final class PhpClassBuilder implements PhpClassBuilderInterface
{
    ...
    public function build(): string
    {
        ...
        if ($this->generateSerializerAttributes) {
            ...
            // Only add SerializedName if any property has a custom JSON key
    ...
    private function buildClassDeclaration(): string
    {
        ...
        // Add extends clause for base class
        if ($this->baseClass !== null) {
            $parts[] = 'extends';
    ...
    /**
     * Build trait use statements inside the class.
     */
    ...
    /**
     * Build the constructor with property promotion.
     */
    ...
    /**
     * Sort properties: required without defaults first, then optional/with defaults.
     *
    ...
    /**
     * Build a single property line for the constructor.
     */
    private function buildPropertyLine(PropertyDefinition $property): string
    {
        $lines = [];

        // Property docblock (if needed)
        ...
        if ($this->generateSerializerAttributes) {
            ...
            // Add SerializedName if JSON key differs from property name
            if ($property->hasCustomJsonKey()) {
    ...
    /**
     * Build property docblock if needed.
     */
    ...
    /**
     * Build the property declaration line.
     */
    ...
    /**
     * Format a default value for PHP code.
     */
    ...
    /**
     * Format an array value for PHP code.
     *
    ...
    /**
     * Build getter methods if enabled.
     */
    private function buildGetters(): string
    {
        ...
        foreach ($this->properties as $property) {
            ...
            // Add docblock for complex types
            if ($property->mappedType->requiresDocblock()) {
                $code .= "    /**\n";
    ...
    /**
     * Build serialization methods (fromArray, toArray, jsonSerialize).
     */
    ...
    /**
     * Build the fromArray factory method.
     */
    ...
    /**
     * Build a property assignment for fromArray method.
     */
    private function buildFromArrayPropertyAssignment(PropertyDefinition $property): string
    {
        ...
        // Determine the extraction expression
    ...
    /**
     * Build the extraction expression for a property in fromArray.
     */
    private function buildFromArrayExtraction(PropertyDefinition $property, string $jsonKey): string
    {
        ...
        // Handle nullable/optional with default
        if ($effectiveDefault !== PropertyDefinition::NO_DEFAULT) {
        ...
        if (in_array($nativeType, ['int', 'float', 'string', 'bool'], true)) {
            // For nullable types with default/null coalescing, we need special handling
            if ($isNullable) {
    ...
    /**
     * Build primitive type coercion expression for nullable types.
     */
    ...
    /**
     * Build primitive type coercion expression.
     */
    ...
    /**
     * Build a property line for toArray method.
     */
    ...
    /**
     * Build the value expression for toArray.
     */
    ...
    /**
     * Build the jsonSerialize method.
     */
    private function buildJsonSerializeMethod(): string
    {
        ...
        // If we have toArray, delegate to it
        if ($this->generateSerialization) {
            $code .= "        return \$this->toArray();\n";
        } else {
            // Generate inline serialization
            $code .= "        return [\n";
            foreach ($this->properties as $property) {
                $code .= $this->buildToArrayPropertyLine($property);
            }
    ...
    /**
     * Add Symfony Validator imports based on property types.
     */
    ...
    /**
     * Build Symfony Validator attributes for a property.
     *
     * @return list<string>
     */
